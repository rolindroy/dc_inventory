	...
	public static function getInstance($config = NULL)
    {
	    if($config === NULL)
	    {
	      if( self::$_instance === NULL ) {
	        self::$_instance = new Connection(TclConfig::config('mysql.dbname'),
	        								  TclConfig::config('mysql.host'),
	        								  TclConfig::config('mysql.username'),
	        								  TclConfig::config('mysql.password'));
			self::$_instance->connect();
	      }
	    }
	    else
	    {
	    	self::$_instance = new Connection($config['dbname'],
	    									  $config['host'],
	    									  $config['username'],
	    									  $config['password']);
	    	self::$_instance->connect();
